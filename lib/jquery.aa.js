	    ...
	    _createClass(_class, [{
	        ...
	        value: function listen(pathWithType, handler) {
	            var _pathWithType$split = pathWithType.split(':'),
	                ...
	                param = _pathWithType$split2[0],
	                type = _pathWithType$split2[1];
	
	            (0, _listenResolver.listenTo)(param, type, handler);
	            return this;
	        }
	    }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	function dispatch(path, type) {
	    ...
	    //pick
	    for (var listenPath in _listenHandlers) {
	        if (!(0, _util.isRelatedPath)(path, listenPath)) {
	            continue;
	        }
	        ...
	        try {
	            for (var _iterator = _listenHandlers[listenPath][Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	...
	function listenTo(path, type, handler) {
